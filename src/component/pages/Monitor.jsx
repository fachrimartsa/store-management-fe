...
// Fungsi untuk mengubah format waktu "HH:MM" menjadi detik
...
export default function Monitor() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (data === "ERROR" || !data || data.length === 0 || data.message === "Tidak ada transaksi ditemukan") {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            const updatedData = currentDataRef.current.map((item) => {
                const key = item.Key;
                ...
                const estimasiDetik = timeToSeconds(item["Estimasi Waktu"]);
                const status = diffSec > estimasiDetik ? "Overtime" : item.Status;
                if(status == "Overtime") {
                  console.log("Mantap");
                }
